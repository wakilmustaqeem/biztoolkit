    ...
    <div className="py-10 px-4">
      <div className="max-w-xl mx-auto bg-white rounded-3xl shadow-xl border border-blue-100 p-8">
        ...
        <div className="space-y-6">
          <div><label htmlFor="fixedCosts" className="block text-sm font-semibold text-gray-700 mb-1">Fixed Costs ($)</label><input id="fixedCosts" type="number" inputMode="decimal" min="0" step="0.01" value={fixedCosts} onChange={(e) => setFixedCosts(e.target.value)} placeholder="e.g. 10000" className="w-full px-4 py-3 border border-blue-200 rounded-xl text-lg text-gray-900 focus:outline-none focus:ring-2 focus:ring-blue-500 bg-blue-50/30" /></div>
          <div><label htmlFor="variableCost" className="block text-sm font-semibold text-gray-700 mb-1">Variable Cost per Unit ($)</label><input id="variableCost" type="number" inputMode="decimal" min="0" step="0.01" value={variableCost} onChange={(e) => setVariableCost(e.target.value)} placeholder="e.g. 20" className="w-full px-4 py-3 border border-blue-200 rounded-xl text-lg text-gray-900 focus:outline-none focus:ring-2 focus:ring-blue-500 bg-blue-50/30" /></div>
          <div><label htmlFor="sellingPrice" className="block text-sm font-semibold text-gray-700 mb-1">Selling Price per Unit ($)</label><input id="sellingPrice" type="number" inputMode="decimal" min="0" step="0.01" value={sellingPrice} onChange={(e) => setSellingPrice(e.target.value)} placeholder="e.g. 50" className="w-full px-4 py-3 border border-blue-200 rounded-xl text-lg text-gray-900 focus:outline-none focus:ring-2 focus:ring-blue-500 bg-blue-50/30" /></div>
          <div><label htmlFor="targetProfit" className="block text-sm font-semibold text-gray-700 mb-1">Target Profit ($) <span className="text-gray-400">(optional)</span></label><input id="targetProfit" type="number" inputMode="decimal" min="0" step="0.01" value={targetProfit} onChange={(e) => setTargetProfit(e.target.value)} placeholder="e.g. 5000" className="w-full px-4 py-3 border border-blue-200 rounded-xl text-lg text-gray-900 focus:outline-none focus:ring-2 focus:ring-blue-500 bg-blue-50/30" /></div>
        ...
        {hasRequired && !isInvalidPricing && <div className="mt-8 bg-gradient-to-r from-blue-50 to-cyan-50 p-6 rounded-2xl border border-blue-100"><h2 className="text-xl font-bold text-gray-800 mb-4">Results</h2><div className="space-y-4"><div className="flex items-center justify-between"><span className="text-gray-600 font-medium">Contribution Margin per Unit</span><span className="text-2xl font-bold text-blue-600">${formatNumber(contributionMargin!)}</span></div><div className="flex items-center justify-between"><span className="text-gray-600 font-medium">Break-even Point (Units)</span><span className="text-2xl font-bold text-green-600">{formatNumber(breakEvenUnits!)} units</span></div><div className="flex items-center justify-between"><span className="text-gray-600 font-medium">Break-even Revenue</span><span className="text-2xl font-bold text-purple-600">${formatNumber(breakEvenRevenue!)}</span></div>{unitsForTargetProfit !== null && <div className="flex items-center justify-between"><span className="text-gray-600 font-medium">Units for Target Profit (${formatNumber(profit)})</span><span className="text-2xl font-bold text-teal-600">{formatNumber(unitsForTargetProfit)} units</span></div>}</div></div>}
      </div>
    </div>
  );
}
