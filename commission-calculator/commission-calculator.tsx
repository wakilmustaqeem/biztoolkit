...
export default function CommissionCalculator() {
  ...
  const commissionAmount = isValid
    ...
    <div className="mx-auto max-w-2xl p-4">
      <div className="rounded-2xl border border-gray-200 bg-white p-6 shadow-lg">
        ...
        <div className="grid gap-4">
          <div>
            <label htmlFor="baseAmount" className="mb-1 block text-sm font-medium text-gray-700">
              Base Amount ($)
            </label>
            <input
              id="baseAmount"
          ...
          <div>
            <label htmlFor="commissionPercent" className="mb-1 block text-sm font-medium text-gray-700">
        ...
        {baseAmount !== "" && commissionPercent !== "" && !isValid && (
          <div role="alert" className="mt-6 rounded-lg border border-red-200 bg-red-50 p-4 text-red-700">
            Base amount and commission percentage must be valid non-negative numbers.
          </div>
        )}

        {isValid && commissionAmount !== null && netPay !== null && (
          <div className="mt-6 rounded-xl border border-gray-200 bg-gray-50 p-4">
            <h2 className="mb-3 text-xl font-semibold text-gray-800">Results</h2>
            <div className="space-y-2">
              <div className="flex justify-between gap-4">
                <span className="text-gray-600">Commission Amount</span>
                <span className="font-bold text-blue-700">${formatNumber(commissionAmount)}</span>
              </div>
              <div className="flex justify-between gap-4">
                <span className="text-gray-600">Total Earnings</span>
                <span className="font-bold text-green-700">${formatNumber(netPay)}</span>
              </div>
            </div>
          </div>
        )}
